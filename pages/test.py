...
@callback(Output('test-field-graph', 'figure'), Input('demo-dropdown', 'value'), Input('match-table', 'active_cell'), prevent_initial_call=True, allow_duplicates=True, suppress_callback_exceptions=True)
...
def create_pitch(value, active_cell):
    ...
    if int(active_cell['row']) < len(df_data):
